...
$query1 = 'SELECT id, titreFr, titreEn, date, corpsFr, corpsEn FROM News ORDER BY date DESC LIMIT 0, 10'; // récupérations des 10 dernières données de la table 'News'
$result1 = mysql_query($query1);

echo '<?xml version="1.0" encoding="iso-8859-1" ?>
<rss version="2.0">
<channel>
<title>Swiss Federation of Tchoukball</title>
<link>http://www.tchoukball.ch/index.php?lien=51</link>
<description>Swiss Federation of Tchoukball</description>
<language>en-En</language>'; // ici on remplit les informations

for ($i = 0; $i < 10 ; $i++) { // on veut 10 afficher news
@$row = mysql_fetch_array($result1); // fonction pour la boucle
if ($row["titreEn"]==""){$titre=$row["titreFr"];} //si le titre anglais n'existe pas
else {$titre=$row["titreEn"];}
if ($row["corpsEn"]==""){$corps="<strong>This news is not available in english</strong><br />".$row["corpsFr"];} //si le contenu anglais n'existe pas
else {$corps=$row["corpsEn"];}

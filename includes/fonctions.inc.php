...
// parametre ==> tableau des elements à afficher
// ligne entière comprend aussi les <tr>
			...
			while ($referee = mysql_fetch_assoc($referees)) {
				...
				if (mysql_query($removingPreviousPointsRecord)) {
					...
					if (!mysql_query($savePointsQuery)) {
						die(printErrorMessage('Problème lors de l\'ajout des nouvelles valeurs.'));
					}
				} else {
					die(printErrorMessage('Problème lors de la suppression des anciennes valeurs.'));
				}
			}

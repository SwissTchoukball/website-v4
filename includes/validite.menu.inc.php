...
if(isset($_GET["menuselection"]) AND is_numeric($_GET["menuselection"])) {
    $menuselection = $_GET["menuselection"];
    if(isset($_GET["smenuselection"]) AND is_numeric($_GET["smenuselection"])) {
...
elseif (isset($_GET[VAR_HREF_LIEN_MENU]) AND is_numeric($_GET[VAR_HREF_LIEN_MENU])) // lien interne au site. pour garder une validite de menu
{
...
$requete = "SELECT * FROM ".$typemenu." WHERE sousMenuDeId=".$menuselection." AND ordre=".$smenuselection."";
$retour = mysql_query("SELECT * FROM ".$typemenu." WHERE sousMenuDeId=".$menuselection." AND ordre=".$smenuselection."");
...
if($typemenu=="MenuAdmin"){ $userLevel = $donnees['userLevel']; }
elseif($typemenu=="Menu"){ $userLevel = 100; }
else{ echo "Le type de menu n'est pas spécifié"; } // ne devrait pas arriver.

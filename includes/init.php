...
if(preg_match('#/~developer#',$_SERVER["SCRIPT_URL"])){
...
if($_SESSION["__userLevel__"]<100 && $admin) { // Menu normal ou menu admin
    $typemenu="MenuAdmin";
}
else {
    $typemenu="Menu";
...
else
{
    ...
    while ($donnees = mysql_fetch_array($retour)){
    	if($menuselection!=1 || $smenuselection!=0 || $admin){ //Pas la page d'accueil.
...
if(isset($newsIdSelection) && $newsIdSelection!=""){ //Page NEWS
	$news = mysql_fetch_assoc(mysql_query("SELECT id, titre".$_SESSION["__langue__"]." AS titre, corps".$_SESSION["__langue__"]." AS corps FROM `News` WHERE `Id`='".$newsIdSelection."'"));
	$titre.= " : ".strip_tags($news['titre']);
	//$description = strip_tags(sizeNewsManager($news['corps'],$tailleDescription,$news['id']));
...
// !Login des admins
if(stristr($PHP_SELF,VAR_HREF_PAGE_ADMIN)!==false && $_SESSION["__adminModeUtilise__"]!==true &&

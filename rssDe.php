...
$query1 = 'SELECT id, titreFr, titreDe, date, corpsFr, corpsDe FROM News ORDER BY date DESC LIMIT 0, 10'; // récupérations des 10 dernières données de la table 'News'
...
if ($row["titreDe"]==""){$titre=$row["titreFr"];} //si le titre allemand n'existe pas
else {$titre=$row["titreDe"];}
if ($row["corpsDe"]==""){$corps="<strong>Die Nachricht ist nicht in französich</strong><br />".$row["corpsFr"];} //si le contenu allemand n'existe pas
else {$corps=$row["corpsDe"];}

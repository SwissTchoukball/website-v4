...
function triDifferenceScorePoints($informations,$tableau,$debug){
	...
	for($k=1;$k<=$tableau[0];$k++){
		if(count($tableau[$k])>1){
			if($debug){
				echo "<br /><strong>Il y a une égalité de points marqués interne.</strong><br />";
			}
			...
			for($i=1;$i<=count($tableau[$k]);$i++){ // Une boucle par équipe à égalité ==>> $i = EQUIPE EGALITE
				$goalaverage[$i]=0; // Initialisation des points interne de l'équipe.
				...
				if($debug){
					echo "<br /><br />ID équipe ".$tableau[$k][$i]." : Goal-average : ".$goalaverage[$i];
				}
			...
			for($m=max($ordningEquipesEgalitesPoints);$m>=min($ordningEquipesEgalitesPoints);$m--){
				...
				for($n=1;$n<=count($tableau[$k]);$n++){
					if($ordningEquipesEgalitesPoints[$n]==$m){
						...
						if($debug){
							echo "<br />nombre de groupes à égalité : ".$groupeEgalite."<br />";
						}
					}
				}
			}

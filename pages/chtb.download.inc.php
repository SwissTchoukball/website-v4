...
if (isset($_GET["download"]) && $_GET["download"] == 'true') {
    ...
    while ($record = mysql_fetch_array($recordset)) {
        ...
        if (!is_file($imageFile)) {
            $imageFile = PATH_DOCUMENTS . $VAR_TABLEAU_DES_LANGUES[0][0] . "_" . substr($record["fichier"], 0,
                    strlen($record["fichier"]) - 3) . "jpg";
        }
        if (!is_file($pdfFile)) {
    ...
    echo "</table>";
    echo "<p align='center'><a href='index.php?" . gardemenuselection($menuselection,
            $smenuselection) . "'>" . VAR_LANG_RETOUR . "</a></p>";
...
<div class="chtb">
    ...
    $textAppondu = $textAppondu . "<p class='consulterArchivesTchoukUp'><a href='?download=true" . gardemenuselection($menuselection,
            $smenuselection) . "'>" . VAR_LANG_CHTB_DOWNLOAD . "</a></p>";
    ...
    while ($text = mysql_fetch_array($recordset)) { // Affichage de tout les paragraphes
        if ($i == 0) { // premier paragraphe = titre + image
            ...
            $imageFile = PATH_DOCUMENTS . $_SESSION["__langue__"] . "_" . substr($record2["fichier"], 0,
                    strlen($record2["fichier"]) - 3) . "jpg";

            // par defaut la premiere langue.
            if (!is_file($imageFile)) {
                $imageFile = PATH_DOCUMENTS . $VAR_TABLEAU_DES_LANGUES[0][0] . "_" . substr($record2["fichier"], 0,
                        strlen($record2["fichier"]) - 3) . "jpg";
            }
            ...
            if ($i == k) { // Deuxième image flottante masquée (il faut mettre un chiffre (correspondant à un §)à la place de k pour afficher)
                ...
                $textAppondu = $textAppondu . "<img class='imageFlottanteGauche' src='" . PATH_DOCUMENTS . $_SESSION["__langue__"] . "_" . substr($record3["fichier"],
                        0, strlen($record3["fichier"]) - 3) . "jpg' alt='" . $record3["fichier"] . "' />";
            }
    ...
    ?>
    <fb:like-box href="http://www.facebook.com/tchoukup" width="600" height="62" show_faces="false" stream="false"

...
$nbCategories = 0;
//$nbTours = 0; //Apparement inutile, � voir si des disfonctionnements arrivent si je mets la ligne en commentaire
$nbTours = mysql_num_rows($retour);
$nbGroupes = 0;
$tableauCategories = array();
if ($nbTours == 0) {
    printBlankSlateMessage('Aucun r�sultats � afficher pour cette saison.');
} else {
    while ($donnees = mysql_fetch_array($retour)) {
        $idTour = $donnees['idTour'];

        if ($tableauCategories[$donnees['idCategorie']] != 'oui') { // On v�rifie si c'est la premi�re fois que cette cat�gorie appara�t dans la liste des tours. Si c'est le cas, l'id de la categorie n'appartient pas � $tableauCategories donc on affiche le nom de la categorie.
        ...
        // $nbTours++; //Apparement inutile, � voir si des disfonctionnements arrivent si je mets la ligne en commentaire
        ...
        if ($nbMatchs == 0) {
            printBlankSlateMessage('Aucun r�sultats � afficher pour cette phase.');
        } else {
            ...
            while ($donneesC = mysql_fetch_array($retourC)) {
                ...
                echo "</td>";
                echo "<td class='center'><a href='/championnat/match/" . $donneesC['idMatch'] . "' title='D�tails'>";

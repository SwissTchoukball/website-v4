...
<?php
    ...
    $nbCategories = 0;
    //$nbTours = 0; //Apparement inutile, ï¿½ voir si des disfonctionnements arrivent si je mets la ligne en commentaire
    $nbTours = mysql_num_rows($retour);
    $nbGroupes = 0;
    $tableauCategories = array();
    if ($nbTours == 0) {
        printBlankSlateMessage('Aucun rï¿½sultats ï¿½ afficher pour cette saison.');
    } else {
        while ($donnees = mysql_fetch_array($retour)) {
            $idTour = $donnees['idTour'];

            if ($tableauCategories[$donnees['idCategorie']] != 'oui') { // On vï¿½rifie si c'est la premiï¿½re fois que cette catï¿½gorie apparaï¿½t dans la liste des tours. Si c'est le cas, l'id de la categorie n'appartient pas ï¿½ $tableauCategories donc on affiche le nom de la categorie.
            ...
            // $nbTours++; //Apparement inutile, ï¿½ voir si des disfonctionnements arrivent si je mets la ligne en commentaire
            ...
            if ($nbMatchs == 0) {
                printBlankSlateMessage('Aucun rï¿½sultats ï¿½ afficher pour cette phase.');
            } else {
                ...
                while ($donneesC = mysql_fetch_array($retourC)) {
                    ...
                    echo "</td>";
                    echo "<td class='center'><a href='/championnat/match/" . $donneesC['idMatch'] . "' title='Dï¿½tails'>";

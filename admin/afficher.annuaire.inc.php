...
if ($_SESSION["__userLevel__"]<=5 && $modificationIdAnnuaire!="") {
    ...
    if ($_POST["action"]=="modifierAnnuaire") {
        ...
        if (mysql_affected_rows() > 1) {
            printMessage("Attention doublon possible : Il existe deja quelqu'un de m&ecirc;me nom et pr&eacute;nom
                          ($nom $prenom). L'insertion continue...");
        }
        ...
        if (mysql_query($requeteSQL)===false) {
            printErrorMessage("Erreur de modification : contactez le webmaster en pr&eacute;cisant les modifications
        ...
        ajouterRelationMultiple(
            "DBDRegroupementAutreFonction",
            "idBDBPersonne",
            "idAutreFonction",
            $idDbdPersonne,
            $DBDRegroupementAutreFonction

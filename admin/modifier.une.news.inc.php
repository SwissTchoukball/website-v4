...
<form name="form1" method="post" action="<?php echo"?menuselection=$menuselection&smenuselection=$smenuselection"; ?>">
    <table class="tableauInsererNews">
        ...
        <tr>
            ...
            <td width=""><input name="titre<? echo $VAR_TABLEAU_DES_LANGUES[$i][0];?>" type="text" size="70" maxlength="80" value="<? echo htmlspecialchars($record["titre".$VAR_TABLEAU_DES_LANGUES[$i][0]],ENT_QUOTES)?>"></td>
        </tr>
        <tr>
            <td valign="top"><p>corps :</p></td>
            <td><textarea name="corps<? echo $VAR_TABLEAU_DES_LANGUES[$i][0];?>" cols="67" rows="20"><? echo $record["corps".$VAR_TABLEAU_DES_LANGUES[$i][0]]?></textarea></td>
        </tr> 
        <tr>
            <td colspan="2">
    ...
    <div class="listeImagesNews">
            Image :<br />
            <input type='radio' name='image' value='false' id='aucun' <? if($record["image"]==0){ echo "checked='checked'"; } ?> /> <label for='aucune'>Aucune</label><br />
        <?
        $retour = mysql_query("SELECT * FROM Uploads WHERE type='jpg' OR type='jpeg' OR type='png' OR type='gif' ORDER BY date DESC");
        while($donnees=mysql_fetch_array($retour)){
            if($record["image"]==$donnees['id']){ $checked=" checked='checked'"; }else{ $checked=""; }
        ...
        <div class="optionsNews">
            <p><input type="checkbox" name="premiereNews" class="couleurCheckBox" <? echo $record["premiereNews"]==0?"":"checked";?>>
  ...
  <p align="center" style="clear: both;"> 
    <input name="action" type="hidden" id="action" value="modifierNews">
		<input name="idNews" type="hidden" id="idNews" value="<? echo $idNewsCourrante;?>">

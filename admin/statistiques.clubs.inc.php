<div id="statsClubs">
	...
	while($donnees=mysql_fetch_assoc($retour)){
		...
		/*if(isset($_GET['save']) && $_SESSION['__userLevel__'] == 0) {
			...
			if (mysql_query($saveStatisticsQuery)) {
				echo "<p class='success'>Statistiques de \"".$donnees['club']."\" enregistrées.</p>";
			} else {
				echo "<p class='error'>Erreur lors de l'enregistrement des statistiques pour \"".$donnees['club']."\".<br />Les statistiques ont peut-être déjà été enregistrées aujourd'hui.</p>";
			}
		}*/
	}
	echo "<tr>";
	...
	if ($_SESSION['__userLevel__'] == 0) {
		?>
		<p><a href="crons/save-clubs-stats.php" target="_blank">Enregistrer les statistiques actuelles</a> (action automatiquement effectuée tous les 1er janvier)</p>
		<?php
	}

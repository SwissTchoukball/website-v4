...
if ($nbActiveMembers != 0) {
	...
	echo $nbActiveMembers.' '.ajouterSSiPluriel("membre",$nbActiveMembers).' '.ajouterSSiPluriel("actif",$nbActiveMembers).
		 ' ('.$statData['nbActiveAdultsMembers'].' '.ajouterSSiPluriel("adulte",$statData['nbActiveAdultsMembers']).' et '.
		 $statData['nbJuniorMembers'].' ' . ajouterSSiPluriel("junior",$statData['nbJuniorMembers']).'), '.
		 $statData['nbActiveMale'].' '.ajouterSSiPluriel("homme",$statData['nbActiveMale']).', ('.round(($statData['nbActiveMale'] / $nbActiveMembers) * 100, 2).'%), '.
		 $statData['nbActiveFemale'].' '.ajouterSSiPluriel("femme",$statData['nbActiveFemale']).', ('.round(($statData['nbActiveFemale'] / $nbActiveMembers) * 100, 2).'%)<br />';

	echo $statData['nbPassiveMembres'].' '.ajouterSSiPluriel("membre",$statData['nbPassiveMembres']).' '.ajouterSSiPluriel("passif",$statData['nbPassiveMembres']).'<br />';
	echo $statData['nbSupportMembres'].' '.ajouterSSiPluriel("membre",$statData['nbSupportMembres']).' '.ajouterSSiPluriel("soutien",$statData['nbSupportMembres']).'<br />';

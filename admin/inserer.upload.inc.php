           ...
           if ($lastPos !== false && in_array(strToLower(subStr($lastPos, 1)), $authorized_extensions)) {
            ...
            if (move_uploaded_file($_FILES['aFile']['tmp_name'],
                ...
                if ($lastPos == '.png' OR $lastPos == '.gif' OR $lastPos == '.jpg' OR $lastPos == '.jpeg') {
                    ...
                    if ($lastPos == '.png') {
                        $miniature = imagepng($destination, $destination_dir_thumb.$thumb_destination_file);
                    }
                    elseif ($lastPos == '.gif') {
                        $miniature = imagegif($destination, $destination_dir_thumb.$thumb_destination_file);
                    }
                    elseif ($lastPos == '.jpg' OR $lastPos == '.jpeg') {
                        $miniature = imagejpeg($destination, $destination_dir_thumb.$thumb_destination_file);
                    }
                    ?>
                        <img src="http://<?php echo $_SERVER["HTTP_HOST"]; ?>/uploads/vignettes/<?php echo $thumb_destination_file; ?>" alt="Miniature de l'image uploadÈe" /><br /><br />
                    <?php

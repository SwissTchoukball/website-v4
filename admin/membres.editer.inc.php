...
<script lang="javascript">
	...
	function checkMemberForm() {
		...
		if(memberEdit.lastname.value.length == 0 && memberEdit.firstname.value.length == 0 && memberEdit.companyName.value.length != 0){
		...
		} else if(memberEdit.lastname.value.length == 0 && memberEdit.firstname.value.length == 0 && memberEdit.companyName.value.length == 0) {
		...
		*/
		...
		//date de naissance
		var dateN = new Date(memberEdit.birthDateYear.value,memberEdit.birthDateMonth.value-1,memberEdit.birthDateDay.value);

		if (memberEdit.birthDateYear.value == 0 && memberEdit.birthDateMonth.value == 0 && memberEdit.birthDateDay.value == 0
		...
		} else if (dateN.getFullYear() != memberEdit.birthDateYear.value || (dateN.getMonth() != memberEdit.birthDateMonth.value-1) || dateN.getDate() != memberEdit.birthDateDay.value){
...
if ($newMember) {
	...
	// Le LIMIT 1 de la requête permet de n'avoir qu'une seule entrée car il pourrait y en avoir plusieurs si le membre a, par exemple, été a plusieurs postes au comité. Pour nous il est juste intéressant de savoir s'il a été au comité, mais pas à quels postes.
	$memberRequest = "SELECT idStatus, derniereModification, modificationPar, idClub, idLangue, idSexe, idCivilite, nom, prenom, adresse, cp, npa, ville, telPrive, telProf, portable, fax, email, emailFSTB, dateNaissance, raisonSociale, idPays, idCHTB, a.idArbitre AS niveauArbitreID, a.descriptionArbitre".$_SESSION['__langue__']." AS niveauArbitre, arbitrePublic, suspendu, typeCompte, numeroCompte, remarque, c.idFonction AS idFonctionComite, cm.idNom AS idCommissionMembre, cn.id AS idCommissionResponsable, cnm.idEquipe AS idEquipeMembre, exp.idPersonne AS idExpert
					  FROM DBDPersonne p
	...
	if (!$memberResult) {
		echo '<p class="error">Erreur lors de la récupération des données du membre.<br />Message : '.mysql_error().'<br />Requête : '.$memberRequest.'</p>';
	} else {
		$member = mysql_fetch_assoc($memberResult);
		if	(($_SESSION['__nbIdClub__'] == $member['idClub'] && $_SESSION["__gestionMembresClub__"]) || $_SESSION['__userLevel__'] <= 5) {
			...
			echo "<br />";
			echo "<p class='error'>Vous n'êtes pas le responsable de la gestion des membres du club de la personne que vous souhaitez éditer.</p>";
		}
	}
}
if ($isSuspended) {
	printMessage($name . ' est suspendu des activités ' . VAR_LANG_ASSOCIATION_NAME . '. Vous ne pouvez donc pas modifier ses informations.');
	$canEdit = false || $_SESSION['__userLevel__'] <= 5;
}
	...
	<form method="post" onsubmit="return checkMemberForm();" name="memberEdit" action="?menuselection=<? echo $menuselection; ?>&smenuselection=<? echo $smenuselection; ?>&details" class="adminForm">
		<fieldset>
			...
			if ($isFSTBVolunteer && $_SESSION['__userLevel__'] > 5) {
				...
				$canEditName = true;
				echo '<span class="infobulle">Donner soit le nom et le prénom, soit la raison sociale, ou bien les deux.</span>';
			}
			?>
		...
		<fieldset>
			<?php
			if ($isCommitteeMember && $_SESSION['__userLevel__'] > 5) {
				$canEditDetails = false;
			} else {
				$canEditDetails = true;
				echo '<span class="infobulle">Au moins la première des deux lignes d\'adresse doit être remplie. Mettre le numéro sur la même ligne que la rue.</span>';
			}
			?>
		...
		<fieldset>
			...
			<select id="statutID" name="statutID" onchange="autoStatutUpdate();">
				...
				if ($dataStatut = mysql_query($queryStatut)) {
					while ($statut = mysql_fetch_assoc($dataStatut)) {
						...
						if ($_SESSION['__userLevel__'] <= 5 || $statut['id'] == 4 || $statut['id'] == 5 || $statut['id'] == 23) {
			...
			if ($refereeLevelId > 1) {
				// Il n'est ainsi pas possible de définir si un membre est suspendu ou arbitre  public lors de son insertion
				// ou lorsqu'il est fait arbitre.

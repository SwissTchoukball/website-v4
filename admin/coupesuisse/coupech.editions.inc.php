<div><a href="?menuselection=<?php echo $_GET['menuselection']; ?>&smenuselection=<?php echo $_GET['smenuselection']; ?>&ajouter"><img src="admin/images/ajouter.png" alt="Ajouter une édition" /> Ajouter une édition</a></div>
<div><a href="?menuselection=<?php echo $_GET['menuselection']; ?>&smenuselection=<?php echo $_GET['smenuselection']; ?>"><img src="admin/images/liste.png" alt="Liste des éditions" /> Liste des éditions</a></div>
<br />
<?php
$nbMaxJournees=10;
...
<form method="post" action="?menuselection=<?php echo $_GET['menuselection']; ?>&amp;smenuselection=<?php echo $_GET['smenuselection']; ?>" class="insererEditionCoupeCH">
	<fieldset>
		...
		<input type="text" size="2" id="nbSetsGagnants" name="nbSetsGagnants" value="<?php echo $nbSetsGagnants; ?>" />

		<label for="pointsPourRemporterSet">Nombre de points pour remporter un set : </label>
		<input type="text" size="2" id="pointsPourRemporterSet" name="pointsPourRemporterSet" value="<?php echo $pointsPourRemporterSet; ?>" />

		<label for="deuxPointsDEcart">Deux points d'écarts nécessaires pour remporter un set </label>
		<input type="checkbox" id="deuxPointsDEcart" name="deuxPointsDEcart" <?php echo $deuxPointsDEcart==1 ? 'checked="checked"':''; ?> />
	...
	<fieldset>
		...
		while($donnees=mysql_fetch_array($retour)){
			...
			<input type="checkbox" id="equipe<?php echo $donnees['idEquipe']; ?>" name="equipe<?php echo $donnees['idEquipe']; ?>"<?php echo $checked; ?> />
			<?php
		}
		?>
	</fieldset>
	<fieldset>
		...
		for($k=1;$k<=$nbMaxJournees;$k++){
			...
			<fieldset id="journee<?php echo $k; ?>" style="<?php echo $style; ?>">
				...
				<input type="text" id="salle<?php echo $k; ?>" name="salle<?php echo $k; ?>" size="30" value="<?php echo $journee['salle'.$k]; ?>" />

				<label for="ville<?php echo $k; ?>">Ville : </label>
				<input type="text" id="ville<?php echo $k; ?>" name="ville<?php echo $k; ?>" size="30" value="<?php echo $journee['ville'.$k]; ?>" />
			</fieldset>
			<?php
		}
		?>
...
<?php
	...
	while($donneesListe=mysql_fetch_array($retourListe)){
		...
		<tr>
			...
			<td class="modifier"><a href="?menuselection=<?php echo $_GET['menuselection']; ?>&smenuselection=<?php echo $_GET['smenuselection']; ?>&modifier=<?php echo $donneesListe['idEvenement']; ?>"><img src="/admin/images/modifier.png" alt="modifier" /></a></td>
			<td class="supprimer"><a href="?menuselection=<?php echo $_GET['menuselection']; ?>&smenuselection=<?php echo $_GET['smenuselection']; ?>&supprimer=<?php echo $donneesListe['idEvenement']; ?>" onClick='return confirm("Vous êtes sur le point de supprimer cette édition \n OK pour supprimer, Annuler pour abandonner.");'><img src="/admin/images/supprimer.png" alt="supprimer" /></a></td>
		</tr>
		<?php
	}
	?>

<div><a href="?menuselection=<? echo $_GET['menuselection']; ?>&smenuselection=<? echo $_GET['smenuselection']; ?>&ajouter"><img src="admin/images/ajouter.png" alt="Ajouter une édition" /> Ajouter une édition</a></div>
<div><a href="?menuselection=<? echo $_GET['menuselection']; ?>&smenuselection=<? echo $_GET['smenuselection']; ?>"><img src="admin/images/liste.png" alt="Liste des éditions" /> Liste des éditions</a></div>
<br />
<?
$nbMaxJournees=10;
...
<script type="text/javascript">
	function updateNbJournees(contenuSelect){
		...
		if(nbJournees < <? echo $nbMaxJournees+1; ?>){
			for(var i=nbJournees; i < <? echo $nbMaxJournees+1; ?>; i++){
				document.getElementById("journee"+i).style.display = "none";
			}
		}
	}
</script>
<form method="post" action="?menuselection=<? echo $_GET['menuselection']; ?>&amp;smenuselection=<? echo $_GET['smenuselection']; ?>" class="insererEditionCoupeCH">
	<fieldset>
		...
		<input type="text" size="2" id="nbSetsGagnants" name="nbSetsGagnants" value="<? echo $nbSetsGagnants; ?>" />
		
		<label for="pointsPourRemporterSet">Nombre de points pour remporter un set : </label>
		<input type="text" size="2" id="pointsPourRemporterSet" name="pointsPourRemporterSet" value="<? echo $pointsPourRemporterSet; ?>" />
		
		<label for="deuxPointsDEcart">Deux points d'écarts nécessaires pour remporter un set </label>
		<input type="checkbox" id="deuxPointsDEcart" name="deuxPointsDEcart" <? echo $deuxPointsDEcart==1 ? 'checked="checked"':''; ?> />
	...
	<fieldset>
		...
		while($donnees=mysql_fetch_array($retour)){
			...
			?>
			<label for="equipe<? echo $donnees['idEquipe']; ?>"><? echo $donnees['nomEquipe']; ?></label>
			<input type="checkbox" id="equipe<? echo $donnees['idEquipe']; ?>" name="equipe<? echo $donnees['idEquipe']; ?>"<? echo $checked; ?> />
			<?
		}
		?>
	</fieldset>
	<fieldset>
		...
		for($k=1;$k<=$nbMaxJournees;$k++){
			...
			?>
			<fieldset id="journee<? echo $k; ?>" style="<? echo $style; ?>">
				<legend>Journée <? echo $k; ?></legend>
					
				<label for="dateJour<? echo $k; ?>">Jour : </label>
				<select id="dateJour<? echo $k; ?>" name="dateJour<? echo $k; ?>" >
					<?
				...
				<label for="dateMois<? echo $k; ?>">Mois : </label>
				<select id="dateMois<? echo $k; ?>" name="dateMois<? echo $k; ?>">
					<?
				...
				<label for="salle<? echo $k; ?>">Salle : </label>
				<input type="text" id="salle<? echo $k; ?>" name="salle<? echo $k; ?>" size="30" value="<? echo $journee['salle'.$k]; ?>" />
				
				<label for="ville<? echo $k; ?>">Ville : </label>
				<input type="text" id="ville<? echo $k; ?>" name="ville<? echo $k; ?>" size="30" value="<? echo $journee['ville'.$k]; ?>" />
			</fieldset>
			<?
		}
		?>	
	...
	<br />
	<input type="hidden" name="idEdition" value="<? echo $idEdition; ?>" />
	<input type="hidden" name="typeEnvoi" value="<? echo $typeEnvoi; ?>" />
...
<?
	...
	while($donneesListe=mysql_fetch_array($retourListe)){
		...
		<tr>
			<td>
				<? echo $donneesListe['annee']; ?>
			</td>
			<td>
				<? echo $donneesListe['nom'.$_SESSION['__langue__']]; ?>
			</td>
			<td class="modifier"><a href="?menuselection=<? echo $_GET['menuselection']; ?>&smenuselection=<? echo $_GET['smenuselection']; ?>&modifier=<? echo $donneesListe['idEvenement']; ?>"><img src="/admin/images/modifier.png" alt="modifier" /></a></td>
			<td class="supprimer"><a href="?menuselection=<? echo $_GET['menuselection']; ?>&smenuselection=<? echo $_GET['smenuselection']; ?>&supprimer=<? echo $donneesListe['idEvenement']; ?>" onClick='return confirm("Vous êtes sur le point de supprimer cette édition \n OK pour supprimer, Annuler pour abandonner.");'><img src="/admin/images/supprimer.png" alt="supprimer" /></a></td>
		</tr>
		<?
	}
	?>

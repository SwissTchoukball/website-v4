				...
				if (!$matchesData = mysql_query($matchesQuery)) {
					...
					if (mysql_num_rows($matchesData) <= 0) {
						...
						while ($match = mysql_fetch_assoc($matchesData)) {
							echo '<li><a href="/championnat/match/'.$match['idMatch'].'">'.$match['equipeA'].' - '.$match['equipeB'].'</a>, '.date_sql2date_joli($match['dateDebut'],'le',$_SESSION['__langue__']).' à '.time_sql2heure($match['heureDebut']).'</li>';
						}
						echo '</ul>';
					}
				}
